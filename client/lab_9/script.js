...
function markered(arr, map){
  ...
  arr.forEach((i,index) => {
    const {coordinates} = i.geocoded_column_1;
...
async function mainEvent() {
  ...
  submit.style.display = ''; // let your submit button disappear
  ...
  if (chartData.length > 0) { // the question mark in this means "if this is set at all"
    ...
    loadAnimation.classList.add('lds-ellipsis_hide');
    ...
    form.addEventListener('input', (event) => { 
      ...
      injectHTML(tlist);
      const localData = shapeData(tlist);
